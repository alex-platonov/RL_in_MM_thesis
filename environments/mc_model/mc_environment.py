...
class MonteCarloEnv(gym.Env, ABC):
    ...
    ----------
    num_levels : int
        Number of price levels to consider in the LOB, i.e. the number of levels above the best bid and best ask to include.
    include_spread_levels : bool
        Flag passed to the LOB class indicating whether spread levels are included in the LOB arrays.
    max_quote_depth : int
        Maximum quote depth in ticks from the best bid and best ask at which the market maker can place orders.
    num_inv_buckets : int
        Number of inventory buckets on each side of the zero-inventory bucket. The total number of buckets is
        2*num_inv_buckets + 1.
    T : int
        Time horizon.
    dt : float
        Time-step size.
    mm_priority : bool
        Flag indicating whether the market maker’s orders receive highest priority. If False, a time-priority system is not
        implemented; instead, the market maker’s orders receive lowest priority.
    num_time_buckets : int
        Number of bins used for the time variable in the state space.
    kappa : int
        Inventory bucket size.
    phi : float
        Running inventory penalty parameter.
    pre_run_on_start : bool
        Flag indicating whether a pre-run is performed when the object is created or when the environment is reset.
    pre_run_iterations : int
        Number of simulation events included in the pre-run.
    MO_action : bool
        Flag indicating whether the market maker is allowed to place market orders.
    debug : bool
        Flag indicating whether debug information is printed during simulation.
    randomize_reset : bool
        Flag indicating whether the LOB is randomised after each reset.
    default_order_size : int
        Order size used for the market maker’s quotes.
    t_0 : bool
        Flag indicating whether time starts at 0 or 1.
    ...
    def load_database(
        ...
        """
        Load database with random LOB states.
    ...
    def set_action_space(self):
        """
        Set the action space. The action space includes the quote depths and the option to place a market order.
        
        Depths are constrained to the integer range from 1 to max_quote_depth. A gym.spaces.box.Box object is created for
        action_space['depths'].
        
        The market-order decision is encoded as a binary indicator (1/0). A gym.spaces.discrete.Discrete object is created for
        action_space['market order'].
        
        Both components are combined into a dictionary action space (gym.spaces.dict.Dict).
    ...
    def _get_action_space_shape(self):
        """
        Return the shape of the action space as a tuple.
    ...
    def state(self):
        ...
        -------
        obs : tuple
            Observation space in terms of (time_varible, inventory_variable).
        """
    ...
    def step(self, action: dict):
        """
        Receive an action (bid and ask depths) and advance the environment by one step.

        Parameters
        ----------
        action : dict
             Action specifying the bid and ask depths.

        Returns
        -------
        obs : tuple
            Observation space expressed as (time_variable, inventory_variable).
        reward : float
            Reward received at time t+1.
        """
        ...
        if self.t < self.T - self.dt:
            ...
            # Bid and ask prices need to be specified before placing orders or this may result in unwillingly changing the orders
            ask = self.mc_model.ob.ask
            bid = self.mc_model.ob.bid

            # If depths are signalled as invalid, adjustments needed
    ...
    def pre_run(self, n_steps=int(1e4)):
        """
        Simulate n_steps events (state transitions) in the Markov-chain LOB.

        Parameters
        ----------
        n_steps : int
            Number of events to simulate in the LOB.
    ...
    def reset(self, randomized=None):
        """
        Reset the environment. Choose a random LOB state if prompted.

        Parameters
        ----------
        randmoized : bool
            A flag to signal if the LOB state should be randomized.
    ...
    def render(self, mode="human"):
        """
        Print useful stats of the environment. Good for debugging. 
...
def init_LOB(num_levels=10, init_ask=10000, init_spread=1, init_volume=1):
    """
    Create and return a neutral LOB state.

    Parameters
    ----------
    num_levels : int    
    Number of bid and ask levels.    
init_ask : in    t
    Initial ask pric    e.
init_spread :     int
    Initial spr    ead.
init_volume     : int
    Initial volume at each depth.
