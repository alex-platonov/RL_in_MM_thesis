...
def tabular_Q_learning(
    ...
    """
    tabular_Q_learning is a function that runs tabular Q-learning.
    
    Parameters
    ----------
    env : object from Class SimpleEnv
        Environment simulating the SPM.
    n : int
    ...
    -------
    Q_tab : defaultdict
        Defaultdict with all Q-values from Q-learning.
    rewards : list
...
def dict_action_to_tuple(dict):
    """
    Translate an action from dict to tuple.
...
def tuple_action_to_dict(tuple):
    """
    Translate an action in tuple to dict.
...
def exponential_decreasing(value, factor=0.9999):
    """
    Return the value at the next time step.
    ...
    value : float
        The current value.
    factor : float
        The multiplicative decay factor (factor < 1).
    
    Returns
    -------
    value : float
        The updated value for the next time step.
    """
...
def load_Q(filename, default=True, folder_mode=False, folder_name=None):
    ...
    default : bool
        Flag indicating whether a defaultdict or a standard dict is returned.
    folder_mode : bool
    ...
    -------
    Q : dict
        A defaultdict or dict containing all Q tables. The keys are actions and the values are the corresponding Q tables.
    args : dict
        A dict with parameters used for the environment.
    rewards_average : list
        A list with rewards received during training.
...
def fetch_table_name(args, n, suffix=""):
    """
    Create a filename based on the input arguments.
    ...
    n : int
        Number of episodes the Q-learning is run for.
    suffix : str
        String appended to the end of the file name.
    ...
    # Creates table names based on the parameters
    return (
        "Q_"
        + "_".join(
            [
...
def plot_Q_zero(Q_zero, x_values):
    """
    Plot Q[(0,0)] over time.
    
    Parameters
    ----------
    Q_zero : np.array
        Array of Q[(0,0)] values.
    x_values : list
        List with episode indices used for plotting.
...
Save parameters to a file.
    ...
    model_parameters : dict
        Dictionary of parameters used for the environment.
    Q_learning_parameters : dict
        Dictionary of parameters used for Q-learning.
    folder_name : str
...
def Q_learning_multiple(
    ...
    """
    Run tabular Q-learning multiple times.
    
    Parameters
    ----------
    args : dict
        Dictionary containing the parameters used for the environment.
    n : int
    ...
    -------
    file_names : list
        List of file names of the saved Q-matrices.
    """
