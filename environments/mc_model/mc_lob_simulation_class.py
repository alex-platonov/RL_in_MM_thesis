...
"""
================== CREDITS =======================
The code below is largely based on the publicly available works of Hanna Hultin, a quant trader from SEB. 
...
class MarkovChainLobModel:
    def __init__(
        ...
        ob_start : numpy array
            Starting limit order book.
        include_spread_levels : bool
            Flag indicating whether spread levels are included in the LOB arrays.
    ...
    def geom_dist_probs(alpha, k=10):
        """
        Compute the first k probabilities of a discrete exponential distribution with parameter alpha.
        
        Parameters
        ----------
        alpha : float
            Parameter of the distribution to sample from.
        k : int
        ...
        -------
        list
            List containing the first k probabilities.
        """
        return (np.exp(alpha) - 1) * np.exp(-alpha * np.arange(1, k + 1))

    def all_rates(self):
        """
        Compute the rates of the different processes.
        
        Returns
        -------
        rates : dict
            Dictionary containing all rates.
        """
        rates = {"mo bid": 0.0467}
    ...
    def update_current_rates(self):
        """
        Extract the rates for all events that are feasible given the current state of the order book.
        
        Returns
        -------
        dict
            Dictionary containing all relevant rates.
        """
    ...
    def sample_next_event(self):
        """
        Sample the next event given the current rates, without applying it to the current order book.
        
        Returns
        -------
        np.array
            The order book state after the sampled transition.
        """
        self.next_event = {}
    ...
    def transition(self, event_dict=None):
        """
        Transition the order book according to a specified event; if no event is provided, an event is sampled from the current rates.
        
        Parameters
        ----------
        event_dict : dict
            Dictionary describing the next event, containing:
                "event" (int)
                "level" (int)
                "abs_level" (int)
                "size" (int)
        ...
        -------
        np.array
            The updated order book after the transition.
        """
    ...
    def mo_size(alpha, k):
        ...
        -------
        s : int
            Size of the sampled market order.
        """
        p = 1 - np.exp(-alpha)
        s = 0
        while s == 0:
    ...
    def simulate(
        ...
        """
        Simulate the order book over a specified number of events.
        ...
        num_print : int
            Frequency (in events) at which the current event number is printed.

        Returns
        -------
        data_dict : dict
            Dictionary containing simulation outputs, including:
                ob : np.array of shape (num_events+1, n)
                    Limit order book at each time step.
                event : np.array
                    Event types at each time step.
                level : np.array
                    Event levels at each time step.
                abs_level : np.array
                    Absolute levels at each time step.
                time : np.array
                    Time between events.
        """
...
num_print :         int
    Frequency (in events) at which the current event number is printed.
        ...
        ---        data_dict : dict
            Dictionary containing simulation outputs, including:
                ob : numpy array of shape (num_events+1, n)
                    Limit order book at each time step.
                event : numpy array
                    Event types at each time step.
                level : numpy array
                    Event levels at each time step.
                abs_level : numpy array
                    Absolute levels at each time step.
                time : numpy array
                    Time between events.vents
        """
    ...
    def run_twap(
        ...
        """
   Run a TWAP that buys the specified total volume, placing trades at fixed time intervals. buy.
        ...
        timefactor : float
            Sampling frequency for saving mid/ask/bid values of the order book.
        initial_ob : numpy array
            Initial limit order book state.
        padding : tuple of two floats
            Time added before the first trade and after the last trade. last

        Returns
        ---        twap_dict : dict
            Dictionary containing information about the TWAP execution. twap
        """
